...
export function buildGovernanceReport(snapshot:Snapshot,profiles:Profile[],assignments:Record<string,{profileId?:string;status?:string;owner?:string;reviewer?:string;note?:string;lastReviewed?:string}>){
 const lists=snapshot.lists.map(list=>{const assignment=assignments[list.id]??null;const profile=(assignment?.profileId?profiles.find(candidate=>candidate.id===assignment.profileId):profiles.find(candidate=>candidate.id===inferProfile(list).id))??null;const checks=profile?audit(list,profile):[];return {listId:list.id,listName:list.name,spaceId:list.spaceId,folderId:list.folderId,assignment,profile:profile?{id:profile.id,name:profile.name,version:profile.version}:null,summary:summarize(checks),findings:checks.filter(check=>check.state!=="pass").map(check=>({key:check.key,label:check.label,state:check.state,area:check.area,reason:check.reason}))};});
 const total=summarize(lists.flatMap(list=>list.findings.map(f=>({...f,actual:null,expected:null,delivery:"manual" as const}))));
 return {schemaVersion:1,kind:"clickup-governance-review",mode:"read-only",createdAt:new Date().toISOString(),snapshotAt:snapshot.capturedAt,workspaceId:snapshot.workspaceId,summary:{lists:lists.length,assigned:lists.filter(list=>list.assignment?.profileId).length,approved:lists.filter(list=>list.assignment?.status==="approved").length,listsNeedingReview:lists.filter(list=>!list.assignment||list.assignment.status!=="approved"||list.summary.drift>0||list.summary.unknown>0).length,drift:lists.filter(list=>list.summary.drift>0).length,unknown:lists.filter(list=>list.summary.unknown>0).length,checks:total},lists};
}
